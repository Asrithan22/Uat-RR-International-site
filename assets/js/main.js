...
(function () {
  ...
  // the 3D story below reads it, so scrolling walks the chapters forward.
  var heroProgress = 0;
  ...
  if (!reduced) {
    ...
    var onFrame = function () {
      ...
      // drives the story; --ho is the closing hand-off to the next section.
      if (heroPin) {
        var hp = 0;
  ...
  if (canvas && canvas.getContext && !reduced) {
    ...
    var t = 0, tAuto = 0, last = 0, visible = true, litPillar = -1;
    ...
    var frame = function (now) {
      ...
      if (visible) {
        // The story runs on its own so the banner is alive at rest. While the
        // banner is held it eases back and the scroll carries it instead —
        // one full pass through the seven chapters over the length of the
        // hold, which is what makes the pillars light in step with the scroll.
        var held = heroProgress > 0.001 && heroProgress < 0.999;
        tAuto += dt * (held ? 0.42 : 1);
        t = tAuto + heroProgress * LOOP * 0.94;   // ends on the star, not past it
        draw();
      }
